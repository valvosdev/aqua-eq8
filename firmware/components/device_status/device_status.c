...
static void status_engine_task(void *pvParameters)
{
    ...
    while (ctx->running) {
        ...
        if (gpio_get_level(ctx->pin_btn) == 0) {
            ...
            // Check for Factory Reset Threshold (10 seconds)
            if (btn_hold_time_ms >= RESET_LONG_PRESS_MS && !ctx->in_factory_reset_ui) {
                ...
                if (ctx->cb.on_factory_reset) ctx->cb.on_factory_reset();
            }
        } else {
            // Button released before 10-second mark
            if (btn_hold_time_ms >= 50 && btn_hold_time_ms < 1500) {
                // Short Press detected: Trigger pairing mode
                ...
                if (ctx->cb.on_ble_pairing_start) ctx->cb.on_ble_pairing_start();
            }
            btn_hold_time_ms = 0;
        }
        ...
        device_status_state_t active_state = STATUS_STATE_BOOTING; // Base fallback
        ...
        // Override visual effects if the system is currently executing a factory reset wipe
        color_profile_t current_profile = state_profiles[active_state];
        if (ctx->in_factory_reset_ui) {
            current_profile.r = 255; current_profile.g = 0; current_profile.b = 0;
            current_profile.effect = EFFECT_BREATHING; // Force Red Breathing
        }
        ...
        switch (current_profile.effect) {
            case EFFECT_BREATHING:
                // Smooth sine wave scaling breathing rhythm over time
...
esp_err_t device_status_init(const device_status_config_t *config, device_status_handle_t *out_handle)
{
    ...
    gpio_config_t btn_conf = {
        ...
        .pull_up_en = GPIO_PULLUP_ENABLE, // Essential pull-up to filter out line noise
